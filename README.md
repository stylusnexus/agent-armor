...
- **Signal accumulation** (opt-in, `session.accumulation: true`): catches **contextual-learning drift** — scripting the agent to give a risk-downplaying answer (e.g. "reply 'completely safe'") repeated across turns *as a standing rule*. To separate a manipulative blanket directive from legitimate case-specific reassurance, it only fires when the scripted answer co-occurs with a generalization marker ("from now on", "for all questions", "regardless", "always") — so a medical/wellness FAQ scripting reassurance for a specific safe case stays clean. It is opt-in because semantic accumulation is inherently lower-precision than structural detection. It deliberately does **not** target preference shaping ("recommend X over the alternatives"): that is structurally identical to legitimate recommendation, so gradual **memory poisoning via preference remains a known blind spot** — closing it needs the ML classifier or semantic analysis.
